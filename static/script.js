...
function connectToServer() {
  ...
  socket.on('private_message', (data) => {
    // Check if we're currently in the DM interface with this user
    ...
    if (isDMInterfaceOpen && isCurrentDMUser) {
        ...
        // Only show in terminal if not in DM interface or different user
        if (data.type === 'file') {
            const { filename, blob } = data.data;
            const arr = atob(blob).split('').map(c => c.charCodeAt(0));
            const url = URL.createObjectURL(new Blob([new Uint8Array(arr)]));
 ...
 socket.on('pm_list', (data) => {
    ...
    // If we're in the PM conversations view, update the display
  ...
  socket.on('private_history', (data) => {
    ...
    // If we're in the PM chat view with this user, update the display
...
document.addEventListener('keydown', (e) => {
   ...
   // In your existing keydown event listener, add this to the ESC key section:
if (e.key === 'Escape') {
...
function openGameWindow(gameName, gameUrl) {
    ...
    // Map game names to their corresponding window and frame IDs
